...
let Y_Po = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
let X_Po = ['8', '7', '6', '5', '4', '3', '2', '1'];
//khai bao su kien
        ...
        return item.map((item) => {
            ...
            if (item) {
                ...
                return {
                    ...
                    value: item.color === "w" ?
                        -table[item.type].value : table[item.type].value,
                    isMoved: move,
                };
            } else {
                return {
...
function getStep(res) {
    $.ajax({
        url: "https://localhost:7044/api/chess/nextstep",
        method: "POST",
        ...
        success: function(data) {
            ...
            if (result) {
                let from = result.currentStep;
                let to = result.nextStep;

                let source = Y_Po[from.y] + X_Po[from.x];
                let target = Y_Po[to.y] + X_Po[to.x];
                game.move({
                    from: source,
                    to: target,
                    promotion: "q",
